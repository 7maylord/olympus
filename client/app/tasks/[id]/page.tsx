...
function StatusTimeline({ task }: { task: ApiTask }) {
  const steps: Array<{ key: string; label: string; ts?: number; active: boolean }> = [
    { key: 'posted', label: 'Task Posted', active: true },
    { key: 'claimed', label: 'Claimed', ts: task.claimedAt, active: !!task.claimedBy },
    { key: 'executed', label: 'Executed', ts: task.executedAt, active: task.status === 'Executed' },
    { key: 'finalized', label: 'Finalized', active: task.status === 'Executed' },
  ];
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0 }}>
            <div
              style={{
                ...
                background: step.active ? 'var(--green)' : 'var(--background-muted)',
                border: `2px solid ${step.active ? 'var(--green)' : 'var(--border)'}`,
                boxShadow: step.active ? '0 0 8px var(--green)' : 'none',
              }}
            />
            {i < steps.length - 1 && (
              <div style={{ width: 2, height: 28, background: step.active ? 'rgba(16,185,129,0.3)' : 'var(--border)', marginTop: 2 }} />
            )}
          </div>
          <div style={{ paddingTop: 1 }}>
            <div style={{ fontSize: '0.825rem', fontWeight: step.active ? 600 : 400, color: step.active ? 'var(--foreground)' : 'var(--foreground-muted)' }}>
              {step.label}
            </div>
            {step.ts && (
...
export default function TaskDetailPage() {
  ...
  if (loadError) {
    return (
      <div className="page-container" style={{ textAlign: 'center', paddingTop: '4rem' }}>
        ...
        <div style={{ fontWeight: 600, marginBottom: '0.5rem' }}>Task not found</div>
        <Link href="/" className="btn-ghost">← Back to tasks</Link>
      </div>
    );
  }
  ...
  return (
    <div className="page-container" style={{ maxWidth: 860 }}>
      {/* Back */}
      <Link href="/" className="btn-ghost" style={{ marginBottom: '1.25rem', display: 'inline-flex' }}>
      ...
      <div className="card" style={{ padding: '1.5rem', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
          <div>
            <div style={{ fontSize: '0.75rem', color: 'var(--foreground-muted)', marginBottom: '0.25rem', fontFamily: 'monospace' }}>
              Task #{task.id}
            </div>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            ...
            <div style={{ fontWeight: 700, fontSize: '1.5rem', color: 'var(--gold)', textShadow: '0 0 12px var(--gold-glow)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem' }}>
          {[
            ...
            { label: 'Expiry', value: new Date(Number(task.expiry) * 1000).toLocaleString() },
            task.claimedBy ? { label: 'Claimed By', value: task.claimedBy, mono: true } : null,
            task.proofHash ? { label: 'Proof Hash', value: `${task.proofHash.slice(0, 16)}…`, mono: true } : null,
            task.latencyMs ? { label: 'Execution Latency', value: `${(task.latencyMs / 1000).toFixed(2)}s` } : null,
          ].filter(Boolean).map((item) => item && (
            <div key={item.label} style={{ padding: '0.75rem', background: 'var(--background-muted)', borderRadius: 'var(--radius-sm)' }}>
              <div style={{ fontSize: '0.7rem', color: 'var(--foreground-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.25rem' }}>
                {item.label}
              </div>
              <div
                style={{ fontSize: '0.8rem', fontFamily: item.mono ? 'monospace' : undefined, wordBreak: 'break-all', cursor: item.mono ? 'pointer' : undefined }}
                onClick={item.mono ? () => handleCopy(item.value) : undefined}
                title={item.mono ? 'Click to copy' : undefined}
              >
                {item.value}
                {item.mono && <Copy size={11} style={{ marginLeft: 4, opacity: 0.5 }} />}
              </div>
            </div>
          ))}
        </div>
